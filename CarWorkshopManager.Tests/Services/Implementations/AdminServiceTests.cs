...
public class AdminServiceTests
{
    ...
    public async Task GetAllUsersAsync_ReturnsListOfUserListItemViewModel()
    {
        ...
        _mockUserManager.Setup(um => um.GetRolesAsync(users[0]))
                        .ReturnsAsync(new List<string> { "Admin" });
        _mockUserManager.Setup(um => um.GetRolesAsync(users[1]))
                        .ReturnsAsync(new List<string> { "Mechanic" });

        var result = await _adminService.GetAllUsersAsync();

        Xunit.Assert.NotNull(result);
        Xunit.Assert.Equal(2, result.Count);
        Xunit.Assert.Contains(result, u => u.Username == "testuser1" && u.FullName == "John Doe" && u.Role == "Admin");
        Xunit.Assert.Contains(result, u => u.Username == "testuser2" && u.FullName == "Jane Smith" && u.Role == "Mechanic");
    }
    ...
    public async Task ChangeUserRoleAsync_AddRoleFails_ReturnsFalse()
    {
        ...
        _mockUserManager.Setup(um => um.FindByIdAsync(userId))
                        .ReturnsAsync(user);
        _mockUserManager.Setup(um => um.GetRolesAsync(user))
                        .ReturnsAsync(new List<string>()); // No current roles
        _mockUserManager.Setup(um => um.AddToRoleAsync(user, newRole))
                        .ReturnsAsync(IdentityResult.Failed(new IdentityError { Description = "Error" }));
